...
def tokens_to_batches(
    dataset, sequence_length, batch_size, output_features, mixture_or_task=None
):
...
def _get_dataset(mixture_or_task_or_name, sequence_length, split, shuffle=True):
...
class HfPyTorchModel(T5Model):
    ...
    def train(
        ...
        for train_step, batch in enumerate(itertools.islice(ds, steps)):
            ...
            outputs = self._model(
                ...
                labels=self.to_tensor(batch["targets"], dtype=torch.long),
            )
            loss = outputs[0]
            loss = loss.mean()
            loss.backward()
    ...
    def predict(
        ...
        *Note*: If a checkpoint step is provided (i.e. `checkpoint_steps is not
        ...
        # elif hasattr(t5.data, "vocabularies") and isinstance(
        #     vocabulary, t5.data.vocabularies.Vocabulary
        # ):
        #     vocabs = {"inputs": vocabulary, "targets": vocabulary}
        ...
        dataset = tokens_to_batches(dataset, sequence_length, batch_size, ["inputs"])
        ...
        for batch in dataset:
            ...
            predicted_tokens = predicted_tokens.cpu().numpy().tolist()
            predictions.extend([vocabs["targets"].decode(p) for p in predicted_tokens])
            print(predictions)
